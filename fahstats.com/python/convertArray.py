...
def convertArray(arr):
	...
	if isinstance(arr, (type([]), type(tuple())) ):		## If the instance is an array
		
		for ele in arr:
			if isinstance(ele, ( type([]), type({}), type(tuple()) )):	## If the instance is an array
		...
		for (k,v) in arr.items():
			item = "'" + str(k) + "'=>"
			if isinstance(v, ( type([]), type({}), type(tuple()) )):
				item += ( convertArray(v))
			else:
